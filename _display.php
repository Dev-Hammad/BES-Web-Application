        ...
        <table style="font-size: small;width:100%" id="example" class="text-nowrap row-border hover order-column table table-bordered table-bordered table-light table-hover table-striped text-center table-responsive-sm  table-responsive-md  table-responsive-lg table-fixed">
            ...
            <tbody id="myTable">
                ...
                if($_GET["fee_status"] == 0)
                {
                    ...
                    WHERE  T2.s_fee_month= '$Get_Data'  AND  T1.s_id = T2.s_id AND T1.Active = 1 AND ( T2.Active = 1 AND T2.s_fee_status = 0 ) ORDER BY T1.s_name DESC";
                }
                else if($_GET["fee_status"] == 1)
                {
                    $List = "SELECT *
                    FROM std T1, std_fee T2
                    WHERE  T2.s_fee_month= '$Get_Data'  AND  T1.s_id = T2.s_id AND T1.Active = 1 AND  ( T2.Active = 1 AND T2.s_fee_status = 1 ) ORDER BY T1.s_name DESC";
                }
                if($_GET["fee_status"] == "NULL")
                {
                    $List = "SELECT *
                    FROM std T1, std_fee T2
                    WHERE  T2.s_fee_month= '$Get_Data'  AND  T1.s_id = T2.s_id AND T1.Active = 1 AND  ( T2.Active = 1 OR T2.s_fee_status = 0 ) ORDER BY T1.s_name DESC";
                }
                ...
                ?>
                    ...
                    ?>
                        <tr class="bg-info text-white">
                            ...
                            ?>
                            <td id="fee"><?php echo $row["s_fee_status"] ?></td>
                            <td>
                                <div class="row p-2">
